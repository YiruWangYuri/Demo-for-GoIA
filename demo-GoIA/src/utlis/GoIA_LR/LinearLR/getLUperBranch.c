...
void getLU(double *data, int numPoint, double epsilon, 
    ...
    for( int index = 0;  index < num_thread; index++) {
        ...
        {
            e_min = data[i + dimension*numPoint];            
            for (int j = 0; j < dimension; ++j) {
                e_min = e_min + getmaxormin(data[i + j*numPoint], subbranch[j + dimension], subbranch[j], -1);
            }            
            
            e_max = data[i + dimension*numPoint];
            for (int j = 0; j < dimension; ++j) {
                e_max = e_max + getmaxormin(data[i + j*numPoint], subbranch[j + dimension], subbranch[j], 1);
            }
            ...
            e0 = data[i + dimension*numPoint];
            for ( int j = 0; j < dimension; ++j ) {
                e0 = e0 - 0.5 * ( subbranch[j] + subbranch[j + dimension] ) * data[i + j*numPoint];
            }
            e0 = fabs(e0);
