function B = imoverlay(A,BW,Numofcolor, colorSpec)
...
if (nargin < 3)
    ...
    B = A;
for i = 1:Numofcolor
    BW_lable = false(sizeA);
    BW_lable(BW==i) = 1;
    BW_lable = repmat(BW_lable,[1 1 3]);
    rgbFill = convertColorSpec(images.internal.ColorSpecToRGBConverter,colorSpec{i});
    rgbFill = im2uint8(rgbFill);
    rgbFill = reshape(rgbFill,[1 1 3]);
    rgbFill = repmat(rgbFill,[sizeA,1]);


    B(BW_lable)  = rgbFill(BW_lable);
end
end
